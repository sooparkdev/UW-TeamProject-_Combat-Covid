...
df_for_map <- cbind(filtered_df, Latitude = us_coords$Latitude,
...
server <- function(input, output) {
  ...
  output$bar <- renderPlotly({
    # compares to find out in what 'age group'
    # the user inputted 'age' falls under
    if (input$age > 84) {
  ...
  col_names <- c("State", "Covid Death Percentage",
                 "Pneumonia Death Percentage",
                 "Influenza Death Percentage",
                 "Total Deaths (inluding other causes)")
  ...
  # Server for the Table
    ...
    output$agebar <- renderPlot({
        age_bar_graph <- ggplot(filtered_df,
                         aes(x = Age.group, y = COVID.19.Deaths)) +
    ...
    sex_df <- health_df %>%
      ...
      group_by(Sex) %>%
      mutate(tot_death = sum(COVID.19.Deaths) +
               sum(Pneumonia.Deaths) + sum(Influenza.Deaths)) %>%
